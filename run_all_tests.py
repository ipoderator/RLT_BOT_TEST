...
def main():
    ...
    if not prerequisites.get('env_file'):
        ...
        print(f"1. Интерактивная настройка: {Colors.GREEN}python setup_env.py{Colors.RESET}")
        print(f"2. Вручную: {Colors.GREEN}cp .env.example .env{Colors.RESET}")
        print(f"   Затем отредактируйте .env и заполните значения")
    ...
    tests = [
        {
            'script': 'test_database_structure.py',
            'description': 'Проверка структуры базы данных',
            'required': ['DATABASE_URL']
        },
        {
            'script': 'test_sql_generation.py',
