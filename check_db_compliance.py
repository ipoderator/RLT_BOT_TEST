...
from database import init_db, Video, VideoSnapshot, Base
...
def main():
    ...
    if is_compliant_videos:
        ...
        for col_name in sorted(details_videos['actual_columns'].keys()):
            print(f"    - {col_name}: {details_videos['actual_columns'][col_name]}")
    else:
        print("✗ Таблица videos НЕ соответствует ТЗ")
        if details_videos.get('missing'):
            print(f"  Отсутствующие колонки: {', '.join(details_videos['missing'])}")
        if details_videos.get('type_mismatches'):
            print("  Несоответствия типов:")
            for mismatch in details_videos['type_mismatches']:
                print(f"    - {mismatch['column']}: ожидается {mismatch['expected']}, "
                      f"фактически {mismatch['actual']}")
        if details_videos.get('extra'):
            print(f"  Дополнительные колонки (не критично): {', '.join(details_videos['extra'])}")
    ...
    if is_compliant_snapshots:
        ...
        for col_name in sorted(details_snapshots['actual_columns'].keys()):
            print(f"    - {col_name}: {details_snapshots['actual_columns'][col_name]}")
    else:
        print("✗ Таблица video_snapshots НЕ соответствует ТЗ")
        if details_snapshots.get('missing'):
            print(f"  Отсутствующие колонки: {', '.join(details_snapshots['missing'])}")
        ...
        if details_snapshots.get('extra'):
            print(f"  Дополнительные колонки (не критично): {', '.join(details_snapshots['extra'])}")
    ...
    for fk in foreign_keys:
        if fk['constrained_columns'] == ['video_id'] and \
           ...
           'id' in fk['referred_columns']:
            ...
            print("✓ Внешний ключ video_snapshots.video_id -> videos.id найден")
            break
